        ...
        .doc(userId)
         ...
         .then(()=>{
           ...
           .set({
             ...
             nameonthecard:this.state.nameonthecard,
             cvv:Math.floor(Math.random()*(1000-100))+100,
             expirydate:this.state.expirydate
           })
         })
...
<Button onPress={()=>this.setState({modalVisible:false,isonline:''})} title="Cancel"></Button>    
      ...
      <Text style={{ fontWeight:'bold',textAlign:'center',paddingTop:20}}>YOUR CREDIT CARD</Text>
  ...
  <Text style={{  fontSize:28,color:'#606060',marginLeft:20,marginTop:70}}>{this.state.cardnumber.replace(/(\d{4})/g, '$1 ').replace(/(^\s+|\s+$)/,' ')}</Text>
  <Text style={{ fontSize:16,color:'#606060',marginLeft:150}}>{this.state.expirydate}</Text>
      ...
      {this.state.status ? <TouchableOpacity onPress={()=>{this.setState({status:false});}}><Text style={{textAlign:'center',fontSize:20,fontWeight:'bold',marginTop:10, color:'blue'}}>hide cvv</Text></TouchableOpacity>:<TouchableOpacity onPress={()=>{this.setState({status:true});}}><Text style={{textAlign:'center',fontSize:20,fontWeight:'bold',marginTop:10, color:'blue'}}>SHOW CVV</Text></TouchableOpacity>}
