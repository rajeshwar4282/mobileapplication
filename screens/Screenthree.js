    ...
    onRefresh = () => {
      ...
      db.collection('notifications')
        ...
        .then((doc)=>{
          ...
          db.collection('Trustworthy')
            .doc(this.state.organization)
            .get().then((doc)=>{
  ...
  try{
     AsyncStorage.getItem('useruid').then((user1)=>{
      ...
      this.setState({userId:user1});
            ...
            .doc(this.state.userId)
            .get().then((doc1)=>{
              this.setState({amount1:doc1.data().amount});
            })
...
pay = ()=>{
 ...
 if(this.state.score > 3.5){
  ...
  this.setState({modalVisible:false});
  this.props.navigation.navigate('transact2')
 }
...
decline = () =>{
  ...
  render() {
      ...
      <Text style={{fontWeight:'bold'}}> {this.state.organization}</Text>
